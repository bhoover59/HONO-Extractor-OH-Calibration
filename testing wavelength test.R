...
# Clear memory
rm(list = ls()) 
# Currently doesn't run because the test file doesn't have cycling --> get NA
...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel('OH Calibration',
             sidebarLayout(
               sidebarPanel(
                 fileInput('raw_data_file', label = 'Raw data file (csv):', multiple = FALSE, placeholder = "No file selected"),
                 strong("Laser Parameters"),
                 fluidRow(
               ...
               mainPanel(
                 ...
                 tabsetPanel(
                   ...
                   tabPanel("View Output Data File", dataTableOutput("table3"))
                 )
               )
             )
    ),
...
server <- function(input, output, session) {
  options(shiny.maxRequestSize = 60*1024^2) # 60 MB max file size upload
  ...
  observeEvent(input$raw_data_file, {
    ...
    df <- read.csv(raw_data_file$datapath, header = TRUE, sep = ",")
    df$DateTime <- as.POSIXct(df$DateTime,  origin = '1904-01-01') # LabView file outputs seconds elapsed since January 1, 1904
  ...
  observeEvent(input$raw_data_file_HONO, {
    ...
    df_HONO <- read.csv(raw_data_file_HONO$datapath, header = TRUE, sep = ",")
    df_HONO$DateTime <- as.POSIXct(as.numeric(df_HONO$DateTime), origin = '1904-01-01 00:00') # LabView file outputs seconds elapsed since January 1, 1904
  ...
  observeEvent(input$OHCalStart, {
    ...
    # MIGHT NEED TO ADD SECTION DELETING BLANK ROWS PRIOR TO CALIBRATION STARTING
    ...
    # Testing for finding new wavelength
      # Finding new wavelength if online is abnormally long (greater than 100)
    WavelengthTest <- aggregate(rawdata$Online, list(rawdata$Cycle), nrow)
    for (k in length(WavelengthTest$x)){
      if(WavelengthTest$x[k] > 100){
        WavelengthTest$Wavelength[k] <- 1
      }
      else{
        WavelengthTest$Wavelength[k] <- 0
      }
    } # 1 means finding new wavelength. 0 means normal cycle. Later in code after averaging for each cycle, subset to only have when equal to 0
    ...
    # Removing when finding new wavelength
    cycles$Wavelength <- WavelengthTest$Wavelength
    cycles <- subset(cycles, cycles$Wavelength != 1)
  ...
  observeEvent(input$HONOExtractorStart, {
    ...
    # Testing for finding new wavelength
    # Finding new wavelength if online is abnormally long (greater than 100)
    WavelengthTest_HONOExtractor <- aggregate(rawdata_HONOExtractor$Online, list(rawdata_HONOExtractor$Cycle), nrow)
    for (k in length(WavelengthTest_HONOExtractor$x)){
      if(WavelengthTest_HONOExtractor$x[k] > 100){
        WavelengthTest_HONOExtractor$Wavelength[k] <- 1
      }
      else{
        WavelengthTest_HONOExtractor$Wavelength[k] <- 0
      }
    } # 1 means finding new wavelength. 0 means normal cycle. Later in code after averaging for each cycle, subset to only have when equal to 0
    ...
    # Removing when finding new wavelength
    cycles_HONOExtractor$Wavelength <- WavelengthTest_HONOExtractor$Wavelength
    cycles_HONOExtractor <- subset(cycles_HONOExtractor, cycles_HONOExtractor$Wavelength != 1)
